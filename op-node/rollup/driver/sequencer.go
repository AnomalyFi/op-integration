...
func NewSequencer(log log.Logger, cfg *rollup.Config, engine derive.ResettableEngineControl, cfgFetcher derive.SystemConfigL2Fetcher, attributesBuilder derive.AttributesBuilder,
	l1OriginSelector L1OriginSelectorIface, nodekit nodekit.QueryService, metrics SequencerMetrics) *Sequencer {
	return &Sequencer{
		log:              log,
		config:           cfg,
		mode:             Unknown,
...
func (d *Sequencer) sealNodeKitBatch(ctx context.Context) (*eth.ExecutionPayload, error) {
	...
	l1Origin, err := derive.NodeKitL1Origin(ctx, d.config, &sysCfg, batch.onto,
...
func (d *Sequencer) startBuildingLegacyBlock(ctx context.Context) error {
	...
	d.log.Info("prepared attributes for new block",
