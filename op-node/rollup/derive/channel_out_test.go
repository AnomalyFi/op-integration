...
func TestBlockToBatchAllTransactionTypes(t *testing.T) {
	...
	l1Info, err := L1InfoDeposit(&defaultTestRollupConfig, eth.SystemConfig{}, 0, eth.HeaderBlockInfo(testutils.RandomHeader(rng)), 0, justification)
	...
	batch, _, err := BlockToSingularBatch(&defaultTestRollupConfig, block)
