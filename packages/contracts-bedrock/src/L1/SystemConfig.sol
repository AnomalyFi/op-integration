...
contract SystemConfig is OwnableUpgradeable, ISemver {
    ...
    ///         When running _without_ NodeKit, the sequencer has a parameter that allows the L1
    ...
    ///         significantly less sensitive to L1 reorgs. With NodeKit, this parameter is not
    ///         supported, since the L1 origin is not up to the sequencer, but must be determined by
    ///         the NodeKit Sequencer. Instead, this `nodekitL1ConfDepth` config can be set and
    ///         used in _both_ the sequencer and the derivation pipeline, to adjust the
    ///         deterministic function of NodeKit L1 block numbers that the sequencer uses to
